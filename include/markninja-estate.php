...
class MarkninjaEstate {
    ...
    public static $otherKeys = array('rect', 'page', 'limit', 'order_type', 'mode');
    ...
    public function handle_ajax() {
        $ajaxPrefixs = array("wp_ajax_", "wp_ajax_nopriv_");
        foreach ($ajaxPrefixs as $ajaxPrefix) {
            add_action( $ajaxPrefix . 'mnj_estate_search', array($this, 'ajax_mnj_estate_search') );
            add_action( $ajaxPrefix . 'mnj_estate_retrieve', array($this, 'ajax_mnj_estate_retrieve') );
            add_action( $ajaxPrefix . 'mnj_estate_list', array($this, 'ajax_mnj_estate_list') );
            add_action( $ajaxPrefix . 'mnj_estate_load', array($this, 'ajax_mnj_estate_load') );
        }
    }
    ...
    public function apply_jss_css_client() {
        if ( !is_admin() ) {
            ...
            wp_enqueue_script( 'mnj_frontend', plugin_dir_url(dirname(__FILE__)) . '/public/js/frontend-script.js', array ( 'jquery' ), '1.0.6', true);
            ...
            wp_enqueue_style( 'mnj_frontend', plugin_dir_url(dirname(__FILE__)) . '/public/css/frontend.css',false,'1.0.6','all');
